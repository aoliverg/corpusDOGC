...
parser = argparse.ArgumentParser(description='A program for classifying Catalan DOGC articles by year.')
...
for r, d, f in os.walk(dirin):
    for file in f:
        try:
            ...
            text="\n".join(lines)
            m = re.search(r'<li>Data del DOGC<br>([0-9][0-9])/([0-9][0-9])/([0-9][0-9][0-9][0-9])</li>',text)
            try:
                data=int(m.group(3))
                print(data)
                if data<1995: dirout="html-pre1995-ca"
                else: dirout="html-"+str(data)+"-ca"
